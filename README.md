...
- 🧪 **生产就绪** - 100% 测试覆盖率（815+ 单元与集成测试），严格类型检查，自动化质量保障

---

## 📚 协作文档

- 贡献指南：[docs/project/CONTRIBUTING.md](docs/project/CONTRIBUTING.md)
- 版本变更记录：[docs/project/CHANGELOG.md](docs/project/CHANGELOG.md)
- 自动升级文档归档：[docs/auto-upgrade/README.md](docs/auto-upgrade/README.md)

## 🗂️ 目录结构（精简后）

项目根目录仅保留必要入口文件，运行/协作文档按职责归档到 `docs/`：

- `docs/project/`：协作流程文档（贡献指南、变更记录）
- `docs/auto-upgrade/`：自动升级与健康检查相关文档
- `docs/tutorials/`：用户教程与使用指引
- `docs/architecture/`：架构说明与目录边界文档
...
# 查看帮助
python3 -m agentsoul.runtime.entry_detect --help
```
...
- 创建完整的 `var/data/` 目录结构（identity/soul/memory）
- 初始化默认 PAD 情感状态
...
**位置：** `apps/web/index.html`

**功能：**
- 📈 **健康度历史趋势** - SVG 折线图展示迭代健康度变化，支持统计分析
- 🎨 **PAD 情绪状态可视化** - 三维条形图实时展示愉悦度/唤醒度/支配度
- 🧪 **健康检查报告可视化** - 导入 `python3 -m agentsoul.health.check --summary-json` 输出的 JSON 文件，查看结构化检查结果
- ✅ **陪伴连续性检查报告可视化** - 导入 `python3 -m agentsoul.health.companionship_checker --summary-json` 输出的 JSON 文件，查看五项核心指标评分
