...
| 工具 | 功能 |
|------|------|
| `collect_interaction_data` | 收集交互数据用于学习 |
| `get_preference_summary` | 获取用户偏好总结 |
| `adjust_pad_state` | 动态调整 PAD 情感状态 |
| `get_learning_stats` | 获取学习统计信息 |

**增强记忆**：
| 工具 | 功能 |
|------|------|
| `enhanced_memory_search` | 增强记忆搜索（支持优先级和标签过滤） |
| `set_memory_priority` | 设置记忆优先级 |
| `add_memory_tags` | 添加记忆标签 |
| `get_memory_by_tags` | 按标签检索记忆 |
