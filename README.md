...
| 健康度可视化 | 健康度历史趋势 SVG/JSON 导出 + 纯静态 Web UI |
...
├── web-ui/
│   └── index.html                # 纯静态 Web 可视化工具 - 查看记忆和情绪历史
├── install.py                    # 安装脚本
└── README.md
```
...
| `get_health_history` | 获取项目迭代健康度历史记录（用于可视化） |
...
| `board_add_labels` | 为项目添加自定义标签分类 |
| `board_remove_labels` | 从项目移除自定义标签 |
| `board_list_labels` | 列出项目所有自定义标签 |
| `board_search_decisions` | 按标签搜索项目决策 |
...
AgentSoul 提供纯静态的 Web UI 用于可视化查看健康度历史、情绪状态和记忆：
