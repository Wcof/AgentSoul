...
export function createCompanionRuntime(options: CompanionRuntimeOptions): CompanionRuntimeService {
  ...
  ensureDefaultState(db, clock);

  return {
    getCompanionRuntimeState() {
      return readRuntimeState(db, clock);
    },
    updateCompanionVitalsAndMood(update) {
      const state = readRuntimeState(db, clock);
      ...
      writeRuntimeState(db, withDerivedVitalBaselines(nextState, clock));
      return nextState;
    },
    ...
    performCompanionInteraction(kind) {
      return performCompanionInteraction(db, kind, clock);
    },
    applyWorkGrowth(input) {
      return applyWorkGrowth(db, input, clock);
    },
    applyGatewayTrafficGrowth(input) {
      return applyGatewayTrafficGrowth(db, input, clock);
    },
    ...
    applyRestRecovery() {
      return applyRestRecovery(db, clock);
    },
    updateSoulAffectiveState(affectiveState) {
      const state = readRuntimeState(db, clock);
      const nextState = {
...
function ensureDefaultState(db: Database.Database, clock: () => Date): void {
  const existing = db
    .prepare("SELECT id FROM companion_state WHERE id = ?")
    .get(DEFAULT_COMPANION_ID);

  if (existing) {
    return;
  }

  writeRuntimeState(db, createDefaultRuntimeState(clock));
}

function readRuntimeState(db: Database.Database, clock: () => Date): CompanionRuntimeState {
  const row = db
    .prepare("SELECT state_json FROM companion_state WHERE id = ?")
    .get(DEFAULT_COMPANION_ID) as { state_json: string } | undefined;

  if (!row) {
    const state = createDefaultRuntimeState(clock);
    writeRuntimeState(db, state);
    return state;
  }

  return applyDerivedVitals(withDefaultGrowthProfile(JSON.parse(row.state_json) as CompanionRuntimeState), clock);
}

function writeRuntimeState(db: Database.Database, state: CompanionRuntimeState): void {
  db.prepare(
  ...
  ).run(state.companion.id, JSON.stringify(state));
}
...
function applyRestRecovery(db: Database.Database, clock: () => Date): CompanionRuntimeState {
  const state = readRuntimeState(db, clock);
...
function writeGrowthEvent(db: Database.Database, event: GrowthEvent): void {
  db.prepare(
...
function listGrowthEvents(db: Database.Database): GrowthEvent[] {
  const rows = db
    .prepare("SELECT event_json FROM growth_events ORDER BY rowid ASC")
    .all() as Array<{ event_json: string }>;

  return rows.map((row) => JSON.parse(row.event_json) as GrowthEvent);
}
