...
export class SoulKVCache {
  ...
  private writeIndex(index: CacheIndex): boolean {
    ...
    if (!checkedPath) {
      ...
      logWAL('kv_cache_write_index', 'index', indexPath, false, { error: 'Path traversal detected' });
      return false;
    }
    ...
    try {
      ...
      const success = true;
      logWAL('kv_cache_write_index', 'index', checkedPath, success, {
        snapshot_count: Object.keys(index.snapshots).length,
      });
      return success;
    } catch (e) {
      console.error('Error writing cache index:', e);
      logWAL('kv_cache_write_index', 'index', checkedPath, false, {
  ...
  private writeSnapshot(snapshot: CacheSnapshot, tier: Tier): boolean {
    ...
    if (!checkedPath) {
      ...
      logWAL('kv_cache_write_snapshot', snapshot.id, snapshotPath, false, { error: 'Path traversal detected' });
      return false;
    }
    ...
    try {
      ...
      fs.writeFileSync(checkedPath, content, 'utf-8');
      const success = true;
      logWAL('kv_cache_write_snapshot', snapshot.id, checkedPath, success, {
        tier,
      ...
      logWAL('kv_cache_write_snapshot', snapshot.id, checkedPath, false, {
        error: (e as Error).message,
      });
      return false;
    }
  ...
  private deleteSnapshot(snapshotId: string, tier: Tier): boolean {
    ...
    if (!checkedPath || !fs.existsSync(checkedPath)) {
      logWAL('kv_cache_delete_snapshot', snapshotId, snapshotPath, false, { error: 'Path not found or traversal' });
      return false;
    }
    ...
    try {
      ...
      const success = true;
      logWAL('kv_cache_delete_snapshot', snapshotId, checkedPath, success, { tier });
      return success;
    } catch (e) {
      console.error(`Error deleting snapshot ${snapshotId}:`, e);
      logWAL('kv_cache_delete_snapshot', snapshotId, checkedPath, false, {
        error: (e as Error).message,
      });
      return false;
    }
