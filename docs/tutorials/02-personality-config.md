...
# 应用到指定配置文件
python -m src.config_manager.cli apply-template creative --target ./my_config.yaml

# 应用时不创建备份
python -m src.config_manager.cli apply-template friendly --no-backup
```
...
# 验证指定配置文件
python -m src.config_manager.cli validate-config --path ./my_config.yaml
```
...
```bash
# 导出到默认位置
python -m src.config_manager.cli export-config

# 导出到指定位置
python -m src.config_manager.cli export-config --output ./backup_config.yaml
```
