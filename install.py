...
def _initialize_identity_data(project_root: Path, config: Dict) -> None:
    persona_cfg = config.get("persona", config.get("agent", {}))
    ai_cfg = persona_cfg.get("ai", {})
    master_cfg = persona_cfg.get("master", {})

    ai_name = str(ai_cfg.get("name", "Agent")).strip() or "Agent"
    ai_nickname = str(ai_cfg.get("nickname", "")).strip()
    ai_role = str(ai_cfg.get("role", "AI Assistant")).strip()
    ai_traits = _normalize_list(ai_cfg.get("personality", []))

    master_name = str(master_cfg.get("name", "")).strip()
    master_nicknames = _normalize_list(
        master_cfg.get("nickname", master_cfg.get("nicknames", []))
    )
    if not master_nicknames:
        master_nicknames = []

    master_timezone = str(master_cfg.get("timezone", "Asia/Shanghai")).strip()
    master_labels = _normalize_list(master_cfg.get("labels", []))
...
def install_mcp(run_after: bool = True, log_path: Optional[str] = None) -> bool:
    ...
    config_loader = ConfigLoader(PROJECT_ROOT)
    try:
        config = config_loader.to_legacy_format()
        _initialize_identity_data(PROJECT_ROOT, {"persona": config})
...
def main():
    parser = argparse.ArgumentParser(description="AgentSoul 人格插件安装脚本")
